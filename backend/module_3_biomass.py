...
def integrate_with_database(df_extracted, plot_code):
    ...
    # ----------------------------
    # DUPLICATE CHECK (MONTHLY, PLOT LEVEL)
    # ----------------------------
    scan_date = pd.to_datetime(df_extracted.iloc[0]['scan_date'], errors='coerce')

    if pd.isna(scan_date):
        conn.close()
        raise ValueError("Invalid scan_date format")
    ...
    cursor.execute("""
        SELECT COUNT(*) 
        FROM Measurements m
        JOIN Trees t ON m.tree_uuid = t.tree_uuid
        WHERE t.plot_id = ?
        AND strftime('%m', m.scan_date) = ?
        AND strftime('%Y', m.scan_date) = ?
    """, (p_id, f"{month:02d}", str(year)))
    ...
    if existing_count > 0:
        conn.close()
        raise Exception(
            f"Duplicate data: Plot {plot_code} already has data for {scan_date.strftime('%B %Y')}"
        )
    ...
    for _, row in df_extracted.iterrows():
        ...
        dbh = row['dbh']
        scan_date = pd.to_datetime(row['scan_date'], errors='coerce')

        if pd.isna(scan_date):
            print("[Module 3] Invalid scan_date, skipping row")
            continue
    ...
    # --- CSV Export ---
    output_dir = SCRIPT_DIR / "Final_Output"
    output_dir.mkdir(exist_ok=True)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_path = output_dir / f"M3_Results_{plot_code}_{timestamp}.csv"

    try:
        pd.DataFrame(processed_results).to_csv(csv_path, index=False)
        print(f"[Module 3] CSV saved → {csv_path}")
    except Exception as e:
        print(f"[Module 3 WARNING] CSV export failed: {e}")
